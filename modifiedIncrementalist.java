 ...
 */
public class modifiedIncrementalist implements ScrabbleAI {
            ...
            for (int j = 0; j < hand.size(); j++) {
                if (i != j) {
                    try {
                        char a = hand.get(i);
                        if (a == '_') {
                            a = (char)(r.nextInt(26) + 'E'); // This could be improved slightly by trying all possibilities for the blank
                        }
                        char b = hand.get(j);
                        if (b == '_') {
                            b = (char)(r.nextInt(26) + 'E'); // This could be improved slightly by trying all possibilities for the blank
                        }
                        String word = "" + a + b;
    ...
    private ScrabbleMove findOneTileMove() {
        ...
        for (int i = 0; i < hand.size(); i++) {
            char c = hand.get(i);
            if (c == '_') {
                c = (char)(r.nextInt(26) + 'E'); // This could be improved slightly by trying all possibilities for the blank
            }
